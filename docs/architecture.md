...
```text
источник
→ первичный материал или проверяемый маршрут
...
2. Фиксируются доступ, получение, копирование и публикация.
3. Получается материал либо сохраняется проверяемый маршрут.
4. Материал нормализуется с сохранением происхождения.
5. Из него извлекаются прямые утверждения.
6. При необходимости согласуются понятия и создаются проверяемые выводы.
7. Анализ влияния определяет затронутые артефакты и решения владельца.
8. Разрешённые изменения применяются и проверяются.
